...
def inv_quad_xfrange(start, stop, step = TILE_SIZE // 5, step_step = 2):
    x = start
    if start > stop:
        while x > stop:
            yield x
...
class Tile:
    def __init__(self, root, canvas: tk.Canvas, xpos, ypos, value, size = TILE_SIZE):
...
class Board(tk.Tk):
    def __init__(self, width, height):
        ...
        # Placeholder buttons for additional functionality
        self.placeholder_button1 = tk.Button(self.button_frame, text="Start Timer", command=self.start_timer)
        self.placeholder_button1.pack(side="left")

        self.placeholder_button2 = tk.Button(self.button_frame, text="Placeholder 2")
        self.placeholder_button2.pack(side="left")
    ...
    def scramble(self, i=0):
        ...
        if i < self.width * self.height * 8: self.after(25, lambda: self.scramble(i + 1))
    ...
    def execute_solution(self):
        ...
        self.after(75, self.execute_solution)
...
def multi_move(a, b):
    print("called multi move")
    for i in range(10):
        game_board.after(i*75, lambda: game_board.move(a, b))
...
game_board.bind("A", lambda _ : game_board.move(0, -1))
game_board.bind("D", lambda _ : game_board.move(0, 1))
game_board.bind("W", lambda _ : game_board.move(1, -1))
game_board.bind("S", lambda _ : game_board.move(1, 1))
game_board.bind("a", lambda _ : game_board.move(0, -1))
game_board.bind("d", lambda _ : game_board.move(0, 1))
game_board.bind("w", lambda _ : game_board.move(1, -1))
game_board.bind("s", lambda _ : game_board.move(1, 1))

game_board.bind("<Control-Right>", lambda _: multi_move(0, 1))
game_board.bind("<Control-Left>", lambda _: multi_move(0, -1))
game_board.bind("<Control-Up>", lambda _: multi_move(1, -1))
game_board.bind("<Control-Down>", lambda _: multi_move(1, 1))

game_board.bind("r", lambda _ : game_board.scramble())

def set_start_time(*_): game_board.start_time = time()
game_board.bind("<space>", set_start_time)
