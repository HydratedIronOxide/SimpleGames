...
class Tile:
    ...
    def __init__(self, canvas: tk.Canvas, root, x_pos, y_pos, value):
        self.canvas = canvas
        self.root = root
        self.x_pos = x_pos
        self.y_pos = y_pos
    ...
    def smooth_show(self, iter_size=5, i=1):
        ...
        else: self.canvas.coords(self.rect_id, x1, y1, x2, y2)  # type: ignore
    ...
    def move_horizontal(self, new_x_pos, iter_dist, i = 1):
        ...
        else: self.root.after(2, lambda: self.move_horizontal(new_x_pos, iter_dist, i + 1))
    ...
    def move_vertical(self, new_y_pos, iter_dist, i = 1):
        ...
        else: self.root.after(2, lambda: self.move_vertical(new_y_pos, iter_dist, i + 1))
...
class Board:
    def __init__(self, canvas, root, starting_board):
        ...
        self.horiz_animation_buffer = 60 * board_x + 100
        self.vert_animation_buffer = 60 * board_y + 100
        self.score = 0
    ...
    def move_left(self):
        ...
        if old_board != self.get_board_state(): 
            number = choices((2, 256), LUCKY_CHANCE)[0]
            self.root.after(200, lambda: self.random_tile(number))
    ...
    def move_right(self):
        ...
        self.check_game_over()
        if old_board != self.get_board_state(): 
            tile = choices((2, 256), LUCKY_CHANCE)[0]
            self.root.after(200, lambda: self.random_tile(tile))
        self.root.after(self.horiz_animation_buffer, lambda: self.reset_animation_flag())
        update_score()
...
def update_score():
    score_label['text'] = f'Score: {game_board.score}'
...
root.title("tk.Tk()")
    # Example usage:
board_x, board_y = map(int, askstring('Size', 'What size should the grid be? Say in terms of X<space>Y').split())  # type: ignore
...
root.bind("<Left>", lambda _:game_board.move_left())
root.bind("<Right>", lambda _:game_board.move_right())
root.bind("<Up>", lambda _:game_board.move_up())
root.bind("<Down>", lambda _:game_board.move_down())
